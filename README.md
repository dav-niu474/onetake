...
| 模板 | 链路 |
|---|---|
| 提示词工坊 · 一键扩写 | 提示词 → LLM 优化 |
| 文生视频 · 极速出片 | 提示词 → 文生视频 → 预览 |
| 图生视频 · 让照片动起来 | 图片上传 → 图生视频 → 预览 |
| 产品展示 · 一图变大片 | 上传 → 图生图 → 图生视频 |
| 全流程 · 提示词工程到成片 | 提示词 → 优化 → 文生图 → 图生视频 → 预览 |
| 口播视频 · 配音合成成片 | 文案 → AI 配音 ∥ 画面生成 → 成片合成 → 预览 |
| **故事分镜 · 三幕成片** | 三幕提示词 → 3×文生视频（并行）→ 三幕拼接 ∥ 旁白配音 → 成片合成 |
